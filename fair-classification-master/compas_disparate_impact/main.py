...
def test_compas_data():
    print("computing p-rule on training set (real data)")
    X, y, x_control = load_data(load_data_size=None, data_path='../data/compas.csv')  # set the argument to none, or no arguments if you want to test with the whole data -- we are subsampling for performance speedup
    ...
    apply_accuracy_constraint = 0
    sep_constraint = 0
    sensitive_attrs_to_cov_thresh = {'race': [0.017, 0.017, 0.017, 0.017, 0.017, 0.017]} # item #0 corresponds to "Afrocan-American"
    print
    ...
    sep_constraint = 0
    gamma = 0.085  # gamma controls how much loss in accuracy we are willing to incur to achieve fairness -- increase gamme to allow more loss in accuracy
