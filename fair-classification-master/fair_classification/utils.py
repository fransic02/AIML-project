...
def print_classifier_fairness_stats(acc_arr, correlation_dict_arr, cov_dict_arr, s_attr_name):
    ...
    count = 0
    non_prot_pos = 0.0
    for k, v in correlation_dict[s_attr_name].items():
        if k != 0.0:
            count += 1
            non_prot_pos += v[1.0]
    non_prot_pos = non_prot_pos/count

    prot_pos = correlation_dict[s_attr_name][0.0][1.0]
...
def compute_p_rule(x_control, class_labels):
    ...
    non_prot_all = sum(x_control != 0.0) # non-protected group
    prot_all = sum(x_control == 0.0) # protected group
    non_prot_pos = sum(class_labels[x_control != 0.0] == 1.0) # non_protected in positive class
